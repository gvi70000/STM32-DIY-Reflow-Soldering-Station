...
extern uint8_t _width;       ///< Display width as modified by current rotation
extern uint8_t _height;      ///< Display height as modified by current rotation
extern uint8_t cursor_x;     ///< x location to start print()ing text
extern uint8_t cursor_y;     ///< y location to start print()ing text
...
void drawLine(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, uint16_t color) {
    if(x0 == x1){
        if(y0 > y1) _swap_int8_t(y0, y1);
        drawFastVLine(x0, y0, y1 - y0 + 1, color);
    } else if(y0 == y1){
        if(x0 > x1) _swap_int8_t(x0, x1);
...
void fillCircleHelper(uint8_t x0, uint8_t y0, uint8_t r, uint8_t corners, uint8_t delta, uint16_t color) {
...
void drawButton(uint8_t x, uint8_t y, uint8_t w, uint8_t h, uint16_t borderColor, uint16_t fillColor, uint16_t txtColor, const char* txt){
	drawRoundRect(x, y, w, h, 4, borderColor);
	fillRoundRect(x+1, y+1, w-2, h-2, 3, fillColor);
